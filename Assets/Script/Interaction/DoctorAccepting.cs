...
public class DoctorAccepting : MonoBehaviour, IInteractable {
  ...
  public void Interact(Interactor player){
    ...
    bool deliveryAccepted = ReceiveItem(player.heldObject.itemName, player.heldObject.itemType);
    if (deliveryAccepted){
      ...
      Debug.Log("Item accepted. Inventory cleared.");
        ...
        Debug.Log($"The doctor rejected the {player.heldObject.itemName}.");
    }
  }
  ...
  public bool ReceiveItem(string itemName, ItemType itemType) {
    if (manager != null){
      ...
      return manager.PlayerSubmittedTool(itemName, itemType);
    }
    return false;
  }
}
